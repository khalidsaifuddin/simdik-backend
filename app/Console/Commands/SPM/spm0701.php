...
class spm0701
{
    static function spm_0701($kode_wilayah, $semester_id){
        $sql_0701 = "
        SELECT
            sekolah.sekolah_id,
            '".$semester_id."' AS semester_id,
            '0701' AS kode_instrumen_spm,
            2 AS target,
            ISNULL( guru.guru_formal, 0 ) AS capaian,
            ( CASE WHEN 2 - ISNULL( guru.guru_formal, 0 ) < 0 then 0 else 2 - ISNULL( guru.guru_formal, 0 ) END )  AS gap,
            (
            CASE	
                WHEN ( ROUND( ( ISNULL( guru.guru_formal, 0 ) / CAST ( ISNULL( 2, 1 ) AS FLOAT ) * 100 ), 2 ) ) >= 100 THEN
                'Tercapai' ELSE 'Belum Tercapai' 
            END 
            ) AS predikat,
            ( CASE WHEN ( ROUND( ( ISNULL( guru.guru_formal, 0 ) / CAST ( ISNULL( 2, 1 ) AS FLOAT ) * 100 ), 2 ) ) > 100 then 100 else ( ROUND( ( ISNULL( guru.guru_formal, 0 ) / CAST ( ISNULL( 2, 1 ) AS FLOAT ) * 100 ), 2 ) ) END ) AS persen,
            'Guru' AS satuan 
        FROM
            sekolah
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_0701);
