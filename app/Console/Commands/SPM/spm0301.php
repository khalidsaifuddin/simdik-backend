...
class spm0301
{
    static function spm_0301($kode_wilayah, $semester_id){
        $sql_0301 = "
        SELECT
            sekolah.sekolah_id,
            '".$semester_id."' AS semester_id,
            '0301' AS kode_instrumen_spm,
            36 AS target,
            ISNULL( sarpras.kursi_siswa, 0 ) AS capaian,
            ( CASE WHEN 36 - ISNULL( sarpras.kursi_siswa, 0 ) < 0 then 0 else 36 - ISNULL( sarpras.kursi_siswa, 0 ) END )  AS gap,
            (
            CASE	
                WHEN ( ROUND( ( ISNULL( sarpras.kursi_siswa, 0 ) / CAST ( 36 AS FLOAT ) * 100 ), 2 ) ) >= 100 THEN
                'Tercapai' ELSE 'Belum Tercapai' 
            END 
            ) AS predikat,
            ( CASE WHEN ( ROUND( ( ISNULL( sarpras.kursi_siswa, 0 ) / CAST ( 36 AS FLOAT ) * 100 ), 2 ) ) > 100 then 100 else ( ROUND( ( ISNULL( sarpras.kursi_siswa, 0 ) / CAST ( 36 AS FLOAT ) * 100 ), 2 ) ) END ) AS persen,
            'Set Meja Kursi Siswa' as satuan
        FROM
            sekolah
            ...
            SELECT
                ...
                SUM( alats.kursi_siswa) as kursi_siswa
            FROM
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_0301);
