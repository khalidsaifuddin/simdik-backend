...
class spm0801
{
    static function spm_0801($kode_wilayah, $semester_id){
        $sql_0801 = "
        SELECT
            sekolah.sekolah_id,
            '".$semester_id."' AS semester_id,
            '0801' AS kode_instrumen_spm,
            ISNULL( guru.guru_target, 0 ) AS target,
            ISNULL( guru.guru_formal, 0 ) AS capaian,
            (
            CASE
                    
                    WHEN ( ISNULL( guru.guru_target, 0 ) - ISNULL( guru.guru_formal, 0 ) ) < 0 THEN
                    0 ELSE ( ISNULL( guru.guru_target, 0 ) - ISNULL( guru.guru_formal, 0 ) )
                END 
                ) AS gap,
                (
                CASE
                        
                        WHEN ( ROUND( ( ISNULL( guru.guru_formal, 0 ) / CAST ( ISNULL( guru.guru_target, 1 ) AS FLOAT ) * 100 ), 2 ) ) >= 100 THEN
                    ...
                    CASE
                            ...
                            WHEN ( ROUND( ( ISNULL( guru.guru_formal, 0 ) / CAST ( ISNULL( guru.guru_target, 1 ) AS FLOAT ) * 100 ), 2 ) ) > 100 THEN
                            100 ELSE ( ROUND( ( ISNULL( guru.guru_formal, 0 ) / CAST ( ISNULL( guru.guru_target, 1 ) AS FLOAT ) * 100 ), 2 ) ) 
                        END 
                        ) AS persen,
                        'Guru' AS satuan 
                    FROM
                        ...
                        SELECT
                            ptkd.sekolah_id,
                            SUM ( 1 ) AS guru_total,
                            ( CASE WHEN ( SUM ( 1 ) * 70 / 100 ) < 1 then 1 else ( SUM ( 1 ) * 70 / 100 ) END ) as guru_target,
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_0801);
