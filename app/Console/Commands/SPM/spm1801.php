...
class spm1801
{
    static function index($kode_wilayah, $semester_id){
        ...
        SELECT
            ...
            '1801' AS kode_instrumen_spm,
            110 AS target,
            110 AS capaian,
            110 - 110 AS gap,
            (
            CASE	
                WHEN ( ROUND( ( 110 / CAST ( 110 AS FLOAT ) * 100 ), 2 ) ) = 100 THEN
                'Tercapai' ELSE 'Belum Tercapai' 
            END 
            ) AS predikat,
            ROUND( ( 110 / CAST ( 110 AS FLOAT ) * 100 ), 2 ) AS persen,
            'Eksemplar' as satuan 
        FROM
            sekolah
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_1801);
