...
class spm1802
{
    static function index($kode_wilayah, $semester_id){
        ...
        SELECT
            ...
            '1802' AS kode_instrumen_spm,
            220 AS target,
            220 AS capaian,
            220 - 220 AS gap,
            (
            CASE	
                WHEN ( ROUND( ( 220 / CAST ( 220 AS FLOAT ) * 100 ), 2 ) ) = 100 THEN
                'Tercapai' ELSE 'Belum Tercapai' 
            END 
            ) AS predikat,
            ROUND( ( 220 / CAST ( 220 AS FLOAT ) * 100 ), 2 ) AS persen,
            'Eksemplar' as satuan 
        FROM
            sekolah
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_1802);
