...
class spm2002
{
    static function index($kode_wilayah, $semester_id){
        ...
        SELECT
            ...
            '2002' AS kode_instrumen_spm,
            34 AS target,
            34 AS capaian,
            34 - 34 AS gap,
            (
            CASE	
                WHEN ( ROUND( ( 34 / CAST ( 34 AS FLOAT ) * 100 ), 2 ) ) = 100 THEN
                'Tercapai' ELSE 'Belum Tercapai' 
            END 
            ) AS predikat,
            ROUND( ( 34 / CAST ( 34 AS FLOAT ) * 100 ), 2 ) AS persen,
            'Minggu/Tahun' as satuan 
        FROM
            sekolah
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_2002);
