...
class spm0201
{
    static function spm_0201($kode_wilayah, $semester_id){
        $sql_0201 = "
        SELECT
            sekolah.sekolah_id,
        ...
        FROM
            ...
            SELECT
                ...
                SUM ( CASE WHEN anggota.jumlah_anggota_rombel <= 32 THEN 1 ELSE 0 END ) AS rombel_kurang_32 
            FROM
        ...
        $fetch = DB::connection('sqlsrv')->select($sql_0201);
        ...
        for ($iFetch=0; $iFetch < sizeof($fetch); $iFetch++) { 
            ...
            echo "[INF] [".($iFetch+1)."/".sizeof($fetch)."] [".$fetch[$iFetch]->kode_instrumen_spm."] [".$fetch[$iFetch]->kode_instrumen_spm."] ".$kode_wilayah.' - '.$fetch[$iFetch]->sekolah_id.' '.$label.PHP_EOL;
